public class PrintedList {
    ...
    void delete(Node delNode) {
        if (head.id.equals(delNode.id)) {//헤드노드가 지울 노드일 경우
            ...
            while (curNode != null) {//노드가 null이기 전까지 (=맨 끝까지)
                if (!curNode.id.equals(delNode.id)) {//curNode가 입력된 노드와 다르다면 다음노드로 이동
